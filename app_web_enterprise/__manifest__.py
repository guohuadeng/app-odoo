...
13. 修复 Odoo 原生 Popover TypeError: Cannot read properties of undefined (reading 'contains')，因 Popover target 销毁后 window pointerdown 监听器未及时清理导致。
    ...
    'assets': {
        ...
        'web.assets_backend': [
            # 本地企业版 web_enterprise 的 tags_list.scss 引用 Odoo 20.1 已删除的 $o-colors-border，
            # 用与核心 web 一致的写法替换（须在依赖 web_enterprise 的本模块中处理）
            ('replace', 'web_enterprise/static/src/core/tags_list/tags_list.scss',
             'app_web_enterprise/static/src/scss/tags_list.scss'),
