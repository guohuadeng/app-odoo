...
class AiRobot(models.Model):
    ...
    provider = fields.Selection(string="AI Provider Name", selection=[('openai', 'OpenAI'), ('azure', 'Azure')],
...
GPT-3	A set of models that can understand and generate natural language
    ...
    def _onchange_provider(self):
        ...
        if self.provider:
            # 取头像（Odoo 19: modules.check_resource_path 已移除，改用 os.path）
            module_path = modules.get_module_path('app_chatgpt', display_warning=False)
            if module_path:
                path = os.path.join(module_path, 'static/description/src/%s.png' % self.provider)
